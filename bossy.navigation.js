            ...
            template:
                '<ul class="navbar">' +
                  '<li class="navbar-item" ng-repeat="menu in menus">' +
                    ...
                    '<a class="navbar-link" ng-if="menu.rootNode.url" href="menu.rootNode.url">{{menu.rootNode.title}}</a>' +
                    ...
                    '<ul class="submenu" ng-show="curOpenMenuIndex===$index">' +
                      // List header only shows when the current node is not the root
                      // Clicking the header sets the current node to its parent.
                      '<lh class="submenu-header" ng-if="menu.curNode!==menu.rootNode" ng-click="menu.curNode=menu.curNode.parentNode"><strong>{{menu.curNode.title}}</strong></lh>' +
                      '<li class="submenu-item" ng-repeat="node in menu.curNode.children">' +
                        // Leaf node (is a link)
                        '<a class="navbar-link" ng-if="node.url" ng-attr-href="{{node.url}}">{{node.title}}</a>' +
